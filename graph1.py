import numpy as np
import matplotlib.pyplot as plt
from matplotlib import font_manager
from matplotlib.ticker import ScalarFormatter

# 设置宋体字体
font_path = "C:/Windows/Fonts/simsun.ttc"
font_prop = font_manager.FontProperties(fname=font_path, size=12)

# g 从 0 到 1
g = np.linspace(0, 1, 1000)

# 各项计算
new_employment = 13000000 * 0.15 * (1 + g) ** 3.03
subsidy = 43.07 * g * new_employment * 12
profit_remaining = 535000000 - subsidy
gdp_growth_value = 2.54 * subsidy

# 找利润剩余 = 0 的临界 g
zero_idx = np.argmax(profit_remaining < 0)
g_zero = g[zero_idx] if zero_idx > 0 else None

# 第一张图：折线图（用不同线型 + 双 y 轴单位）
fig, ax1 = plt.subplots(figsize=(10, 6))

# 设置左轴格式
ax1.yaxis.set_major_formatter(ScalarFormatter(useMathText=False))

# 左轴：单位为“人”的量
l1, = ax1.plot(g, new_employment, linestyle='-', label="新就业人数")
ax1.set_ylabel("人数（人）", fontproperties=font_prop)
ax1.tick_params(axis='y')

# 右轴：单位为“元”的量
ax2 = ax1.twinx()
ax2.yaxis.set_major_formatter(ScalarFormatter(useMathText=False))  # 添加这行
l2, = ax2.plot(g, subsidy, linestyle='--', label="补贴额")
l3, = ax2.plot(g, profit_remaining, linestyle='-.', label="利润剩余")
l4, = ax2.plot(g, gdp_growth_value, linestyle=':', label="GDP增长值")
ax2.set_ylabel("金额（元）", fontproperties=font_prop)
ax2.tick_params(axis='y')

# 标注零点位置
if g_zero:
  ax1.axvline(x=g_zero, color='gray', linestyle=':', linewidth=1)
  ax1.text(g_zero, ax1.get_ylim()[1], f"g ≈ {g_zero:.3f}", rotation=0, ha='center', va='bottom',
           color='black', fontproperties=font_prop)

# 图例合并，放在 ax1 上避免 tight_layout 冲突
lines = [l1, l2, l3, l4]
labels = [line.get_label() for line in lines]
ax1.legend(lines, labels, loc='upper left', prop=font_prop)

plt.xlabel("g（补贴率）", fontproperties=font_prop)
plt.grid(True)
plt.tight_layout()
plt.show()

# 第二张图：面积堆叠图（利润剩余占比、GDP增长率）
import numpy as np
import matplotlib.pyplot as plt
from matplotlib import font_manager
from matplotlib.ticker import ScalarFormatter

# 设置宋体字体
font_path = "C:/Windows/Fonts/simsun.ttc"
font_prop = font_manager.FontProperties(fname=font_path, size=12)

# g 从 0 到 1
g = np.linspace(0, 1, 1000)

# 各项计算
new_employment = 13000000 * 0.15 * (1 + g) ** 3.03
subsidy = 43.07 * g * new_employment *  12
profit_remaining = 535000000 - subsidy
gdp_growth_value = 2.54 * subsidy

# 找利润剩余 = 0 的临界 g
zero_idx = np.argmax(profit_remaining < 0)
g_zero = g[zero_idx] if zero_idx > 0 else None

profit_ratio = profit_remaining / 535000000 * 100  # %
gdp_growth_rate = gdp_growth_value / 126058200000000 * 100  # %

# 限定范围
profit_ratio_clipped = np.clip(profit_ratio, -100, 100)
gdp_growth_rate_clipped = np.clip(gdp_growth_rate, 0, 0.01)

# 创建图
fig, ax1 = plt.subplots(figsize=(10, 6))

# 左轴：利润剩余占比
p1 = ax1.fill_between(g, profit_ratio_clipped, label="利润剩余占比", hatch='////', facecolor='none', edgecolor='black')
ax1.set_ylabel("利润剩余占比（%）", fontproperties=font_prop)
ax1.set_ylim(-100, 100)


# 右轴：GDP增长率
ax2 = ax1.twinx()
p2 = ax2.fill_between(g, gdp_growth_rate_clipped, label="GDP增长率", hatch='\\\\', facecolor='none', edgecolor='blue')
ax2.set_ylabel("GDP增长率（%）", fontproperties=font_prop)
ax2.set_ylim(0, 0.01)

# 标注g_zero时的GDP增长率
if g_zero:
    gdp_at_g_zero = gdp_growth_rate_clipped[np.argmin(np.abs(g - g_zero))]
    ax2.text(g_zero, gdp_at_g_zero, f"g = {g_zero:.3f}\nGDP增长率 ≈ {gdp_at_g_zero:.4f}%",
             rotation=0, ha='left', va='bottom', color='black', fontproperties=font_prop, 
             backgroundcolor='white', fontsize=12, fontweight='bold')

# 图例合并
handles = [p1, p2]
labels = [h.get_label() for h in handles]
ax1.legend(handles, labels, loc='upper left', prop=font_prop)

plt.xlabel("g（增长率）", fontproperties=font_prop)
plt.title("第二张图：面积堆叠图（利润剩余占比 vs GDP增长率）", fontproperties=font_prop)
plt.grid(True)
plt.tight_layout()
plt.show()

